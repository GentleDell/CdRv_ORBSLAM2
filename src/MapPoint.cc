...
{
    unique_lock<mutex> lock(mMutexFeatures);
    if(mObservations.count(pKF))    // map.count():返回key出现的次数，由於在map里映射唯一，返回只能是0和1
        return;
    // 记录下能观测到该MapPoint的KF和该MapPoint在KF中的索引
    mObservations[pKF]=idx;

    if(pKF->mvuRight[idx]>=0)   // 双目或者grbd
        nObs+=2;
    else
        nObs++; // 单目
}
...
void MapPoint::SetBadFlag()
{
    map<KeyFrame*,size_t> obs;
    {
        ...
        mObservations.clear();  // 把mObservations指向的内存释放，obs作为局部变量之后自动删除————1.操作mObservations要加锁，尽量少占用时间;2.此后还要更新相关的KF;
    }
