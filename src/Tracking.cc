    ...
    mpFrameDrawer(pFrameDrawer), mpMapDrawer(pMapDrawer), mpMap(pMap), mnLastRelocFrameId(0)
{
    ...
    //     |fx  0   cx| fx fy 指的是感光器件在x和y方向上 1毫米包含的像素數
    // K = |0   fy  cy| cx cy 是指光心在x y方向的位置
    //     |0   0   1 |
    ...
    else// 步骤2：跟踪
    {
        ...
        if(!mbOnlyTracking)
        {
            ...
            if(mState==OK)
            {
                ...
                // 更新Fuse函数和SearchAndFuse函数替换的MapPoints 參考幀和當前幀重復的點，用參考幀的替換掉
                ...
                if(mVelocity.empty() || mCurrentFrame.mnId<mnLastRelocFrameId+2) // 獲取可靠的MM至少兩幀 ？
                {
                    // 将上一帧的位姿作为当前帧的初始位姿
        ...
        else
        {
            ...
            else
            {
                ...
                if(!mbVO)
                {
                    ...
                    if(!mVelocity.empty())
                    {
                        bOK = TrackWithMotionModel();   // 不加後續判斷，那麼MM不好的時候就直接到後面變成LOST,進行重定位（但是不是应该加上？）
                    }
                    else
                    {
                        bOK = TrackReferenceKeyFrame();
                ...
                else
                {
                    ...
                    // mbVO为1，则表明此帧匹配了很少的3D map点，少于10个，既做跟踪又做重定位
                    ...
                    if(!mVelocity.empty())
                    {
                        ...
                        // 这三行没啥用？  ———— 是不是重定位中會修改Matches，然後如果重定位失敗但MM成功，就靠這一步恢復MM的時候用的Matches
                    ...
                    if(bOKMM && !bOKReloc)
                    {
                        ...
                        if(mbVO)
                        {
                            ...
                            for(int i =0; i<mCurrentFrame.N; i++)
                            {
                                ...
                                {
                                    mCurrentFrame.mvpMapPoints[i]->IncreaseFound();     // 增加能看到該點的幀數
                                }
                            }
                        }
                    }
        ...
        // 跟踪失败，并且relocation也没有搞定，只能重新Reset  清空地圖，關鍵幀等系統數據————但是只有当前地图数据少时才会重置
        if(mState==LOST)
        {
    ...
    if(mCurrentFrame.N>500)
    {
        ...
        for(int i=0; i<mCurrentFrame.N;i++)
        {
            ...
            if(z>0)
            {
                ...
                // a.观测到该MapPoint的关键帧
                // b.该MapPoint的描述子
                // c.该MapPoint的平均观测方向和深度范围  在Track local map 用到

                // a.記錄该MapPoint可以被哪个KeyFrame的哪个特征点观测到
...
bool Tracking::NeedNewKeyFrame()
{
    ...
    if(mCurrentFrame.mnId<mnLastRelocFrameId+mMaxFrames && nKFs>mMaxFrames) // 略微控制插幀速度？
        return false;
    ...
    // 和關鍵幀相比跟踪不行了，0.25和0.3是一个比较低的阈值
    const bool c1c =  mSensor!=System::MONOCULAR && (mnMatchesInliers<nRefMatches*0.25 || bNeedToInsertClose) ;
    // Condition 2: Few tracked points compared to reference keyframe. Lots of visual odometry compared to map matches.
    // 阈值比上一條件要高，与之前参考帧（最近的一个关键帧）重复度不是太高
    ...
    while(nCandidates>0 && !bMatch)
    {
        for(int i=0; i<nKFs; i++)
        {
            ...
            if(!Tcw.empty())
            {
                ...
                if(nGood<50)
                {
                    int nadditional =matcher2.SearchByProjection(mCurrentFrame,vpCandidateKFs[i],sFound,10,100);  // sFound 記錄已經找到的點
                        ...
                        if(nGood>30 && nGood<50)
                        {
                            ...
                            for(int ip =0; ip<mCurrentFrame.N; ip++)
                                if(mCurrentFrame.mvpMapPoints[ip])
                                    sFound.insert(mCurrentFrame.mvpMapPoints[ip]);  // 更新已經找到的點的集合，再從剩下的點中放寬條件找一次
