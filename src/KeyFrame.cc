 ...
 *
 * 计算mBowVec，并且将描述子分散在第4层上，即mFeatVec记录了属于第i个node的ni个描述子 不精確到葉結點可能是爲了減少運算量，到第四層的精度就夠了？
    ...
    for(int i=0; i<N; i++)
    {
        ...
        if(pMP)
        {
            if(!pMP->isBad())
            {
                if(bCheckObs)
                {
                    if(mvpMapPoints[i]->Observations()>=minObs) // 该MapPoint是一个高质量的MapPoint
                        nPoints++;
                }
                else
                    nPoints++;
...
{   
            ...
            mvpMapPoints[i]->EraseObservation(this);    // 让与自己有联系的MapPoint删除与自己的联系
    {
        ...
        while(!mspChildrens.empty())
        {
            ...
            if(bContinue)
            {
                // 因为父节点擦除了，并且子节点找到了新的父节点，子节点更新自己的父节点
                pC->ChangeParent(pP);
